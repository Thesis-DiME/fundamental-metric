...
class CLIPSimilarity(Metric):
    ...
    def __init__(
        self, mode: str = "text_image", model_name: str = "ViT-B/32", **kwargs
    ):
        super().__init__(**kwargs)

        assert mode in ["text_image", "image_image"], (
            "Mode must be 'text_image' or 'image_image'"
        )
        self.mode = mode
    ...
    def update(self, sources: List[str], targets: List[Image.Image]) -> None:
